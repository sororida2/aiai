...
- `services/<name>/workflow.py` — `@workflow_step`/`@judged`로 고정 파이프라인 선언, `@tool`로 최상위 capability 등록
...
- `@workflow_step` 함수는 항상 `func(context: dict[str, Any]) -> str` 형태. 반환 문자열은 그 스텝의 `next={...}` 맵의 키와 정확히 일치해야 한다 — 안 맞으면 `StateMachine.run()`이 `ValueError`로 즉시 죽는다.
- `@judged` 함수도 시그니처는 동일하게 `func(context) -> str`이지만, 반환값이 `choices=(...)` 밖이면 데코레이터가 `ValueError`를 던진다. `@workflow_step`과 이중으로 씌워 state machine에는 보통 노드와 구분 없이 등록한다 (`manual_review` 참고).
...
- `judged` 위반은 별도 클래스 없이 평범한 `ValueError`다 — 다른 세 예외와 달리 커스텀 타입이 아니므로, 호출부에서 구분해서 잡아야 한다면 메시지 프리픽스(`"judged node '...' returned"`)로 식별한다.
...
- `inferred` 값은 판단 분기(**`@judged` 이외의 곳**)에서 그대로 쓰면 안 된다 — `require_confirmed()`를 거치지 않고 코드 로직에서 `confidence`를 확인하지 않은 채 분기하면 안티패턴. `fetch_status`는 `result["status_confidence"] != "confirmed"`를 직접 체크해 `"미확인"` outcome으로 `manual_review`(judged 노드)에 위임하는 걸 표준 패턴으로 삼는다.
...
- [ ] inferred 매핑이 판단 분기(judged 이외)에 쓰이지 않는가
