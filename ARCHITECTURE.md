...
shared/countries.py         ← (2026-08-07 추가) ISO 3166-1 alpha-2 국가 코드 frozenset — 여러 서비스가
                                공유하는 순수 참조 데이터. framework/(엔진)도 아니고 특정 서비스 소유도
                                아니라 별도 최상위 디렉토리. public_holiday/university_search가 input_schema
                                검증에 재사용(§ harness/guardrail.py의 validate_tool_input() 절)
...
**(2026-08-07 추가) `validate_tool_input(tool_name, kwargs)`** — `validate_tool_output()`과 대칭인 input 쪽 opt-in 헬퍼. `registry.tool_for(tool_name).input_schema`(`@tool(input_schema=...)`로 선언)를 `validate_schema()`에 위임하고, 위반 시 `GuardrailViolation(stage="input", ...)`을 던진다 — tool 함수 본문 맨 앞에서 명시적으로 불러야 한다. SDK가 함수 시그니처로 하는 타입 체크(str/int 등 형태만)로는 못 잡는 **의미** 검증(예: `country_code`가 실제 유효한 alpha-2 코드인가)이 필요한 tool만 선언한다 — 특히 다단계 tool 체이닝에서 모델이 이전 tool의 출력으로 다음 tool의 인자를 스스로 합성하는 경우, 사용자가 직접 준 값이 아니라 검증 없이 흘려보내면 위험이 크다는 점에서 착안했다. 실제 사례: `public_holiday`/`university_search`가 `country_code`를 `shared/countries.py`의 `ISO_3166_1_ALPHA2`(이 프로젝트가 이미 alpha-2로 통일시킨 "국가" 정준 표현, § `limitation.md`의 "업계 비교" 절)로 검증한다 — `university_search`는 원래도 자기 `mapping.json`의 `SemanticMapping.normalize()`가 우연히 이 역할을 하고 있었지만, `public_holiday`는 Nager.Date가 alpha-2를 그대로 받아 정규화 매핑 자체가 없어서 잘못된 값이 들어와도 여태 전혀 안 걸러지고 있었다 — 이게 이 opt-in input 검증을 추가한 실제 동기다.
...
- `subscription_weather_flow`가 내부에서 직접 호출하는 `subscription_status()`/`weather()`는 SDK `Runner`를 거치지 않으므로 Agent 단위 `output_schema_guardrail`의 대상이 안 된다(§ "서비스를 조합하는 서비스" 절) — 다만 각 tool 본문 안의 `validate_tool_output()`(및, 그 tool이 선언했다면 `validate_tool_input()`)은 그대로 실행되므로 검증 자체가 우회되는 건 아니다. SDK 마이그레이션으로 guardrail이 tool 단위에서 Agent 단위로 넘어가면서 "input을 **boundary**(Runner가 tool을 부르기 직전)에서 가로채 검증한다"는 개념 자체는 없어졌지만(§ 위 "SDK 마이그레이션" 절 4번 항목), **(2026-08-07 추가)** `public_holiday`/`university_search`처럼 실제로 필요한 tool은 이제 `validate_tool_input()`을 함수 본문 맨 앞에서 opt-in으로 불러 의미 검증을 한다(§ 위 `harness/guardrail.py` 절) — SDK의 타입 체크만으로는 못 잡는 부분을 이걸로 메운다.
