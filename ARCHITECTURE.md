...
`@guardrail(input_schema=..., output_schema=...)`의 dict 값 자리에 `registry.output_schema_for(name)`/`registry.input_schema_for(name)`을 넣으면, 다른 서비스의 스키마를 재사용하면서도 그 값의 실제 평가는 검증 시점(tool이 호출될 때)까지 늦춰진다 — `tool_for()`가 함수 조회를 호출 시점으로 늦추는 것과 같은 이유. 이름을 `output_schema_for`/`input_schema_for` 둘로 분리한 이유는, 하나의 이름(`schema_for`)이 input/output 중 뭘 주는지 애매해지면 반대 스키마를 조용히 가져오는 실수가 생길 수 있어서다. `output_schema`/`input_schema` 자체(최상위)는 여전히 평범한 dict이고, 이 헬퍼들이 돌려주는 thunk가 그 dict 안의 **개별 값**으로 들어갈 뿐이다 — 자세한 동작은 § 아래 `harness/guardrail.py`/`서비스를 조합하는 서비스` 절 참고.
...
### `orchestrator.py` — 라우팅 + 인자 추출 + 일시정지/재개
...
3. `spec.input_schema`에서 호출자가 이미 준 `kwargs`에 없는 필드만 추려 `AgentRunner.extract_arguments(request, tool_name, missing_schema, known=kwargs)` 호출 — `missing_schema`가 비어 있으면(기존처럼 kwargs를 전부 채워서 부른 경우) 이 단계 자체를 안 타서 100% 하위호환. 추출된 값은 `{**extracted, **kwargs}`로 병합되는데 **호출자가 준 값이 항상 우선**이라 추측값이 명시값을 덮어쓸 수 없다.
4. 선택된 tool을 `GuardrailChain.run()`으로 감싸 실행, 그 안에서 `tracer.span()`으로 Tool Span 기록
...
**`extract_arguments()` — 인자 추출.** `AgentRunner`에 `choose_tool()`의 짝으로 추가됐다(`framework/orchestrator.py`). 구현은 둘 다 있다:
- `OpenAIRunner.extract_arguments()` — 실제 LLM 호출로 JSON을 뽑아낸다. 프롬프트에 "확신 없는 필드는 키 자체를 넣지 마라(null도 쓰지 마라)"고 명시해서, 억지로 틀린 값을 채우는 대신 **비워두게** 유도한다 — 그러면 `spec.func(**kwargs)`가 파이썬의 "필수 인자 없음" `TypeError`로 명확하게 실패한다(조용히 잘못된 값이 들어가는 것보다 나음).
- `FirstMatchRunner.extract_arguments()` — 오프라인 스텁이라 숫자형 필드만 요청 문자열에서 정규식으로 뽑는 거친 휴리스틱. 문자열형 필드(국가 코드 등)는 진짜 의미 이해가 필요해서 이 스텁 몫이 아니다.
...
`output_schema`/`input_schema` **자체**는 항상 진짜 dict다 — lambda는 호출부(서비스 파일)에 전혀 안 보인다. 대신 그 dict **안의 개별 값**이 인자 없는 callable(thunk)일 수 있다: `harness/schema.py`의 `validate_schema()`가 재귀 검증 중 각 필드의 `expected_type`이 dict가 아니면서 `callable`이면 그 자리에서(=검증 시점, tool이 실제 호출될 때) 평가해서 풀어낸다. `ToolRegistry.output_schema_for(name)`/`input_schema_for(name)`(`registry/decorators.py`)이 바로 이 thunk를 만들어 돌려주는 헬퍼다 — 내부에서 각각 `lambda: self.guardrail_for(name).output_schema`/`.input_schema`를 리턴하지만, 그 lambda는 헬퍼 **안에 감춰져** 있어서 호출부는 `registry.output_schema_for("weather")`라고만 쓰면 된다(§ "서비스를 조합하는 서비스" 절 실제 예시). 이렇게 하면:
- dict 자체는 데코레이터 적용 시점에 바로 만들어지므로(진짜 dict라서), `@guardrail(output_schema=...)`의 인자 자체가 이상해질 일이 없다.
- 그 안의 thunk 값만 검증 시점까지 평가가 늦춰지므로, `discover_services()`가 서비스를 어떤 순서로 import하든(참조 대상이 아직 등록 전이어도) 안전하다.
- `harness/schema.py`는 이 thunk가 registry를 참조한다는 사실을 전혀 모른다 — "callable이면 부른다"는 제네릭한 규칙만 알아서, 스키마 검증기와 tool registry 사이의 결합이 생기지 않는다.

(주의: `typing.Any`도 `callable(Any)`가 `True`를 반환한다 — `validate_schema()`는 `expected_type is Any` 체크를 먼저 통과시켜 걸러낸 뒤에만 이 callable 분기에 도달하므로 충돌하지 않지만, 새로 스키마 관련 코드를 만질 때는 이 순서를 유지해야 한다.)
...
- output 스키마도 이제 직접 import하지 않는다: `@guardrail(output_schema={"subscription": registry.output_schema_for("subscription_status"), "weather": registry.output_schema_for("weather")})`. `output_schema` 자체는 이 자리에서 즉시 만들어지는 **진짜 dict**다 — 다만 그 안의 값(`registry.output_schema_for(name)`)은 `lambda: self.guardrail_for(name).output_schema`를 돌려주는 thunk이고, `harness/schema.py`의 `validate_schema()`가 재귀 검증 중 그 값이 dict가 아니라 callable이면 그 자리에서(=검증 시점, tool이 실제 호출될 때) 평가해서 풀어낸다(§ `harness/guardrail.py` 절). lambda는 이 헬퍼 안에 감춰져 있어서 이 파일 어디에도 `lambda`라는 글자가 없다 — `registry.tool_for()`가 함수 조회를 호출 시점으로 늦추는 것과 같은 원리를 스키마에도 적용한 것. 대칭으로 `input_schema_for(name)`도 있다(§ 위 `registry/decorators.py` 절 — 지금은 실제로 쓰는 서비스가 없음).
...
위 예시는 `applicant_id`를 호출자가 이미 채워서 `extract_arguments()`가 아예 안 불린다. kwargs 없이 순수 자연어만 던지면 그 단계가 추가된다:

```
orchestrator.handle("미국에 있는 대학 5개만 보여줘")
  └─ agent_runner.choose_tool(...) → "university_search"
     └─ missing_schema = {"country_code": "str"}   ("country_code"가 kwargs에 없어서)
        └─ agent_runner.extract_arguments(request, "university_search", missing_schema, known={})
           → {"country_code": "US"}   (OpenAIRunner가 실제 LLM으로 뽑아낸 값)
        └─ kwargs = {**{"country_code": "US"}, **{}}  → {"country_code": "US"}
     └─ guardrails.run("university_search", spec.func, {"country_code": "US"})
        └─ spec.func(country_code="US") 호출 → 이후는 위와 동일한 골격
```
...
- **(해결됨, 참고로 남김)** `subscription_weather_flow`가 다른 서비스를 호출할 때 함수는 registry 조회로 바꿨지만(§ 위 절), output 스키마 상수(`SUBSCRIPTION_STATUS_OUTPUT_SCHEMA`/`WEATHER_OUTPUT_SCHEMA`)는 한동안 여전히 직접 import했다 — `@guardrail(output_schema=...)`가 모듈 로드 시점에 그 값을 필요로 하는데, `discover_services()`가 아직 모든 서비스를 등록 못 했을 수 있어(알파벳 순서상 `subscription_weather_flow`가 `weather`보다 먼저 import됨) 즉시 registry에서 읽으면 `None`을 가져오는 버그가 났다. 처음엔 `output_schema=lambda: {...}`(스키마 전체를 감싸는 lambda)로 고쳤는데, 사용자가 "lambda 자체를 안 보이게 할 수 없냐"고 재차 요청해서 한 단계 더 다듬었다 — `harness/schema.py`의 `validate_schema()`가 재귀 검증 중 **각 필드 값**이 dict가 아니라 callable이면 그 자리(검증 시점)에서 평가하도록 일반화하고, `ToolRegistry.output_schema_for(name)`이 그 lambda를 내부에 감춰서 돌려주는 헬퍼가 됐다(대칭으로 `input_schema_for(name)`도 있음 — 사용자가 "이름 하나가 input/output 중 뭘 주는지 애매해진다"고 지적해서 하나로 합치지 않고 처음부터 둘로 분리함). 그래서 지금은 `output_schema={"subscription": registry.output_schema_for("subscription_status"), "weather": registry.output_schema_for("weather")}`처럼 쓰고, 이 파일 어디에도 `lambda`가 안 보인다 — dict 자체는 즉시 만들어지는 진짜 dict이고 그 안의 값만 나중에 풀리기 때문. `harness/schema.py`는 이 값이 registry를 참조한다는 걸 전혀 모른다("callable이면 부른다"는 제네릭 규칙만 앎) — 스키마 검증기와 tool registry 사이에 새 결합이 생기지 않았다.
...
- "목록에서 이름을 보고 다음 요청에 applicant_id를 넣는" 연결은 프롬프트 문구(`prompts/applicant_list.md`)로만 안내할 뿐, 실제로 이름→ID를 찾아 다음 tool 호출의 인자를 채우는 로직은 어디에도 없다. `AgentRunner.extract_arguments()`(§ 위 `orchestrator.py` 절)가 생기면서 "인자까지 추출하는 `AgentRunner` 구현체"라는 전제 자체는 채워졌지만, 이 특정 시나리오는 여전히 안 풀린다 — `extract_arguments()`가 보는 건 그 turn의 `request` 텍스트와 이미 확정된 `kwargs`뿐이라, "직전 turn에 `applicant_list`가 보여준 표에서 김민준이 A101이었다"는 사실을 알 방법이 없다(대화 히스토리/이전 결과를 참조하는 경로가 없음). 이걸 풀려면 이전 turn의 결과를 다음 `extract_arguments()` 호출에 컨텍스트로 넘기는 별도 배선이 필요하다 — 아직 안 함.
